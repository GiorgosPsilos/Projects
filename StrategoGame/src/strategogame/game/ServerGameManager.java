...
import strategogame.game.board.ServerBoard;
...
public class ServerGameManager implements Runnable {
     ...
     * 
     * @see strategogame.game.ClientGameManager
     */
    @Override
    public void run() {
        createIOStreams();
        ...
        try {
            ...
            for (int row = 0; row < 3; ++row) {
                for (int col = 0; col < 10; ++col) {
                    board.getSquare(row, col).setPiece(setupBoardOne.getPiece(2 - row, 9 - col));
                    board.getSquare(row + 5, col).setPiece(setupBoardTwo.getPiece(row, col));
                    
                    if(setupBoardOne.getPiece(2 - row, 9 - col).getPieceType() == PieceType.FLAG)
                    	playerOneFlag = new Point(row, col);
                    if(setupBoardTwo.getPiece(row, col).getPieceType() == PieceType.FLAG)
                    	playerTwoFlag = new Point(row + 5, col);
                }
            }
            ...
            for (int row = 0; row < 1; ++row) {
                for (int col = 0; col < 10; ++col) {
                    ...
                    setupBoardOne.setPiece(setupBoardOne.getPiece(2 - row, 9 - col), row, col);
                    setupBoardOne.setPiece(temp, 2 - row, 9 - col);
                    
                    // Player Two
                    temp = setupBoardTwo.getPiece(row, col);
                    setupBoardTwo.setPiece(setupBoardTwo.getPiece(2 - row, 9 - col), row, col);
                    setupBoardTwo.setPiece(temp, 2 - row, 9 - col);
                }
            }
        ...
        while (true) {
            try {
                ...
                if (playerOne.getColor() == turn) {
                	...
                	move.setStart(7-move.getStart().x, 9-move.getStart().y);
                	move.setEnd(7-move.getEnd().x, 9-move.getEnd().y);
                }
                else{
                ...
                if(board.getSquare(move.getEnd().x, move.getEnd().y).getPiece() == null) {
                	...
                	moveToPlayerOne.setStart(new Point(7 - move.getStart().x, 9 - move.getStart().y));
                	moveToPlayerOne.setEnd(new Point(7 - move.getEnd().x, 9 - move.getEnd().y));
    ...
    private ArrayList<Point> computeValidMoves(int row, int col, PieceColor inColor) {
    	...
    	if(pieceType != PieceType.TRAP && pieceType != PieceType.FLAG) {
	    	// Negative Row (UP)
	    	for(int i = -1; i >= -max; --i) {
