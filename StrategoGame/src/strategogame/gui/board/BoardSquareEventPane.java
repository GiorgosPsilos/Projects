...
import strategogame.game.GameStatus;
import strategogame.game.MoveStatus;
import strategogame.game.Piece;
import strategogame.game.PieceColor;
import strategogame.game.PieceType;
import strategogame.game.board.ClientSquare;
import strategogame.gui.board.setup.SetupPanel;
import strategogame.gui.board.setup.SetupPieces;
import strategogame.media.ImageConstants;
import strategogame.util.HashTables;
...
public class BoardSquareEventPane extends GridPane {
    ...
    private class OnHover implements EventHandler<MouseEvent> {
        ...
        public void handle(MouseEvent e) {
            ...
            if (null != Game.getStatus()) // Setting up
                switch (Game.getStatus()) {
                    case SETTING_UP -> checkMove(row, col, hover);
                    case WAITING_OPP -> invalidMove(hover);
                    case IN_PROGRESS -> {
                        if(null != Game.getMoveStatus())
                            switch (Game.getMoveStatus()) {
                                case OPP_TURN -> invalidMove(hover);
                                case NONE_SELECTED -> checkMove(row, col, hover);
                                case START_SELECTED -> {
                            }   
                                default -> {
                            }
    ...
    private class OffHover implements EventHandler<MouseEvent> {
        ...
        public void handle(MouseEvent e) {
            ...
            {   // Change the behavior of the off hover based on the game/move status
                switch (Game.getStatus()) 
                {
                case SETTING_UP -> hover.setImage(ImageConstants.HIGHLIGHT_NONE);
                case WAITING_OPP -> hover.setImage(ImageConstants.HIGHLIGHT_NONE);
                case IN_PROGRESS -> {
                    if(null != Game.getMoveStatus())
                        switch (Game.getMoveStatus()) {
                            case OPP_TURN -> hover.setImage(ImageConstants.HIGHLIGHT_NONE);
                            case NONE_SELECTED -> hover.setImage(ImageConstants.HIGHLIGHT_NONE);
                            case START_SELECTED -> {}
                            default -> {}
                        }
    ...
    private boolean isHoverValid(int row, int col) {
        ...
        if (Game.getStatus() == GameStatus.SETTING_UP && row <= 4)
            return false;
